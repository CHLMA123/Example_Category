...
@interface XJQRCodeViewController ()<AVCaptureMetadataOutputObjectsDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UIAlertViewDelegate>
{
    NSString *QRCode;
}
...
@implementation XJQRCodeViewController
...
/*
 ...
 在扫描二维码时会调用captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection这个代理方法,
 这时如果涉及到扫描数据成功并跳转至另一个页面这个功能的实现,那么在扫描到数据后必须调用[self.session stopRunning]这个方法停止扫描,不然会导致captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection这个代理方法一直调用,导致页面跳转次数>=1,会形成页面错乱(尤其是在block中调用跳转页面的方法更容易出现这个问题)
 为了解决这个问题可以:(看下面)
 */
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection{
    ...
    if (metadataObjects.count > 0) {
        ...
        if (obj) {
            ...
            NSString *str =  [ [obj stringValue] stringByRemovingPercentEncoding];//如果扫描出被编码的数据需要解码
            
//            NSDictionary * dic =[NSDictionary dictionaryWithDictionary:[self parseJSONStringToNSDictionary:str]];//字典格式的字符串转化为字典
            
            if (str.length>5) {//扫描成功
                ...
                [self dismissAlertView:str];//扫描失败
            }
...
- (void)dismissAlertView:(NSString *)message {
    
    WS(weakSelf)
    if (IOS8UP) {
        
        UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"扫描失败" message:message preferredStyle:UIAlertControllerStyleAlert];
        ...
        ;
        UIAlertAction *okAction = [UIAlertAction actionWithTitle:@"再次扫描" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
    ...
    }else
    {
        ...
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"扫描失败" message:message delegate:self cancelButtonTitle:@"关闭" otherButtonTitles:@"再次扫描", nil];
        [alert show];
    }
